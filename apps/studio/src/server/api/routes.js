...
import {
  ...
  loadSnapshotNetwork,
  syncFilesystem
} from "../artifacts/inventory.js";
import { loadSimilarityResult } from "../services/similarity.js";
  ...
  return async function handleApi(request) {
    ...
    try {
      if (request.method === "GET" && pathname === "/api/health") {
        return sendJson({ ok: true, service: "transit-lab-studio", eventSchemaVersion: RUN_EVENT_SCHEMA_VERSION });
      }
      ...
      if (request.method === "GET" && pathname === "/api/artifacts") {
        return sendJson(listArtifacts(db, url.searchParams.get("limit")));
      }
      ...
      if (request.method === "GET" && pathname === "/api/runs") {
        return sendJson(listRuns(db, url.searchParams.get("limit")));
      }
      if (request.method === "POST" && pathname === "/api/runs") {
        ...
        return sendJson(createRun(db, spec), 202);
      }
