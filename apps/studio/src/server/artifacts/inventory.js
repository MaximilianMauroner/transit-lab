...
import { fingerprint, validateArtifactManifest, validateDatasetManifest, validateInferenceResult } from "../../shared/contracts/index.js";
import {
  all,
  dataRoot,
  json,
...
async function syncExplicitArtifactManifest(db, root, manifestPath) {
  ...
  const files = (manifest.files || []).map((file) => ({ ...file, path: relativePath(root, resolve(root, file.path)) }));
  if (files.some((file) => !inside(root, resolve(root, file.path)))) return null;
