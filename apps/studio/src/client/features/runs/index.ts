import { api, escapeHtml, formatDate, shortId, statusClass } from "../../api.ts";
import { errorCard, loading, sectionHeading, status } from "../../components/ui.ts";
    ...
    const showDetail = async (runId) => {
      ...
      try {
        ...
        if (["queued", "claimed", "running"].includes(current.status)) {
          stream = new EventSource(`/api/control/runs/${encodeURIComponent(runId)}/events?after=${Math.max(-1, (current.events || []).at(-1)?.seq ?? -1)}`);
