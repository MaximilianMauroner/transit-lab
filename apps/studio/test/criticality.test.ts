...
import {
  ...
  primaryMetricName
} from "../src/client/features/criticality/normalize.ts";
...
test("normalizes the Rust prediction contract and line names", () => {
  const dataset = normalizePredictionFile({
    ...
    predictions: [{ line: 7, metrics: [0.125, 95], structural_uniqueness: 0.4 }]
  });
...
test("accepts named metric objects and keeps generated values aligned", () => {
  const dataset = normalizePredictionFile({
    ...
    predictions: [{
      lineId: "U1",
      metricValues: { accessibility_loss: 0.2, unreachable_share: 0.1 },
...
test("rejects malformed prediction rows", () => {
  expect(() => normalizePredictionFile({ predictions: "not an array" })).toThrow("predictions array");
