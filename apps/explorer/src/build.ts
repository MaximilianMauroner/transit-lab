...
async function buildStyles(outdir: string) {
  ...
  const process = Bun.spawn([tailwind, "-i", input, "-o", output, "--minify"], { stdout: "pipe", stderr: "pipe" });
  const [stdout, stderr] = await Promise.all([
    new Response(process.stdout).text(),
    new Response(process.stderr).text()
  ]);
  const exitCode = await process.exited;
  if (exitCode !== 0) throw new Error(`Tailwind build failed (${exitCode}): ${stderr || stdout}`);
}

export async function buildExplorerClient(outdir = resolve(import.meta.dir, "../public/dist")) {
