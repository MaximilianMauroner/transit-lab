...
import { fingerprint, validateArtifactManifest, validateDatasetManifest, validateInferenceResult } from "../../contracts/src/index.ts";
import {
  all,
  dataRoot,
  json,
...
async function syncExplicitArtifactManifest(db, root, manifestPath) {
  ...
  for (const file of files) {
    const artifactCandidate = resolve(artifactRoot, file.path);
    const legacyCandidate = resolve(root, file.path);
    const path = await Bun.file(artifactCandidate).exists() ? artifactCandidate : legacyCandidate;
    if (!inside(artifactRoot, path) && !inside(root, path)) return null;
...
async function syncInferenceResultFile(db, root, path, snapshotById) {
  ...
  const artifact = indexedArtifactForPath(db, root, path, ["inference-result"]) || await ensureArtifact(db, root, {
    kind: "inference-result",
    path,
    fingerprintValue: fingerprint("inference-result-v1", result),
...
async function syncModelAndPredictions(db, root, modelPath, predictionsPath, snapshotById) {
  ...
  const modelId = predictions?.modelId ?? predictions?.model_id ?? "model-demo-reference";
  const checkpointArtifact = indexedArtifactForPath(db, root, modelPath, ["model-checkpoint"]) || await ensureArtifact(db, root, {
  ...
  const inferenceId = `inference-${modelId}-${snapshotId}`;
  ...
  for (const prediction of predictions.predictions || []) {
    const lineInstanceId = `${snapshotId}:${Number(prediction.line)}`;
