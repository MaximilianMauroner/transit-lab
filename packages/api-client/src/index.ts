function scopedPath(path) {
  if (path.startsWith("/api/") && !path.startsWith("/api/public/") && !path.startsWith("/api/control/")) {
    return `/api/control${path.slice("/api".length)}`;
  }
  return path;
}

export async function api(path, options: RequestInit = {}) {
